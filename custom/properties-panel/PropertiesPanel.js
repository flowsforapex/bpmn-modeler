...
var createDropdown = require('bpmn-js-properties-panel/lib/Utils').createDropdown,
...
PropertiesPanel.prototype.attachTo = function(parentNode) {
  ...
  this._bindParentListeners(parentNode);
...
PropertiesPanel.prototype._bindParentListeners = function(parentNode) {

  let m_pos, w;

  domDelegate.bind(parentNode, '.properties-panel-parent', 'mousedown', function(e) {
    if (e.offsetX < 5) {
      m_pos = e.x;
  ...
  domDelegate.bind(parentNode, 'div', 'mouseup', function() {
  ...
  function resize(e) {
    const dx = m_pos - e.x;
    m_pos = e.x;
    w = (parseInt(getComputedStyle(parentNode, '').width) + dx);
    if (w > 260) {
      parentNode.style.width = w + "px";
      parentNode.firstChild.style.width = w + "px";
    }
  }
}
