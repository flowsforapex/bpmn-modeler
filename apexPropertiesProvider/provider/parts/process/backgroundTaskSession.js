...
function getBusinessObjectCustom(element) {
...
function refreshApplications(element) {
  ...
  getApplications().then((values) => {
    ...
    if (values) {
      applications = values;
      // get property value
      property = getBusinessObjectCustom(element).get('applicationId') || null;
...
function refreshPages(element, applicationId) {
  ...
  getPages(applicationId).then((values) => {
    ...
    if (values) {
      pages = values;
      // get property value
      property = getBusinessObjectCustom(element).get('pageId') || null;
      // add entry if not contained
...
function refreshUsernames(element) {
  ...
  getUsernames().then((values) => {
    ...
    if (values) {
      usernames = values;
      // get property value
      property = getBusinessObjectCustom(element).get('username') || null;
      // add entry if not contained
      if (
        property != null &&
...
export default function (
  ...
  if (is(element, 'bpmn:Process') || is(element, 'bpmn:Participant')) {
    ...
    group.entries.push(
      entryFactory.selectBox(translate, {
        ...
        get: function (element) {
          var bo = getBusinessObjectCustom(element);
    ...
    applicationSelectBox = entryFactory.selectBox(translate, {
      ...
      hidden: function () {
        return getBusinessObjectCustom(element).manualInput === 'true';
      },
      get: function (element) {
        var bo = getBusinessObjectCustom(element);
    ...
    group.entries.push(
      entryFactory.textField(translate, {
        ...
        hidden: function (element) {
          return getBusinessObjectCustom(element).manualInput === 'false';
        },
        get: function (element) {
          var bo = getBusinessObjectCustom(element);
          var value = bo.get('applicationId');
